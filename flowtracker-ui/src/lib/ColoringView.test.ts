...
test('add coloring', async () => {
  ...
  expect(coloring.assignments[0]).toMatchObject({color: autoColors[0], selections: [selection1]})
})
...
test('add selection to coloring', async () => {
  ...
  expect(coloring.assignments[0]).toMatchObject({color: autoColors[0], selections: [selection1, selection2]})
})
...
test('remove selection from coloring', async () => {
  ...
  render(ColoringView, {coloring, selection: selection1})
  await user.click(getSquare(0))
  expect(coloring.assignments[0]).toMatchObject({color: autoColors[0], selections: [selection2]})
})
